...
hero_new = """<!-- SECTION 1 — HERO SECTION -->
...
compliance_new = """<!-- ADDITIONAL SECTION — COMPLIANCE & SAFETY STANDARDS -->
        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition">
                    ...
                    <p class="text-sm text-gray-600 font-body">Structured packing, layered cushioning, and fragile tagging system.</p>
                </div>
                <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition">
                    <div class="w-12 h-12 bg-[#0B1B3D] text-[#D4AF37] rounded-xl flex items-center justify-center text-xl mb-6">
                    ...
                    <p class="text-sm text-gray-600 font-body">Protection against unforeseen transit risks and damages.</p>
                </div>
                <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition">
                    <div class="w-12 h-12 bg-[#0B1B3D] text-[#D4AF37] rounded-xl flex items-center justify-center text-xl mb-6">
                    ...
                    <p class="text-sm text-gray-600 font-body">Trained professionals with strict accountability standards.</p>
                </div>
                <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition">
                    <div class="w-12 h-12 bg-[#0B1B3D] text-[#D4AF37] rounded-xl flex items-center justify-center text-xl mb-6">
                    ...
                    <p class="text-sm text-gray-600 font-body">Real-time tracking and delivery confirmation processes.</p>
                </div>
            </div>
        </div>
    </section>
...
enterprise_pricing = """
    <!-- SECTION 3 — ENTERPRISE CUSTOM SOLUTION -->
    <section class="py-24 bg-[#0B1B3D] relative overflow-hidden">
        ...
        <div class="max-w-4xl mx-auto px-6 text-center relative z-10">
            ...
            <a href="contact.html" class="inline-flex btn-primary bg-[#D4AF37] text-[#0B1B3D] hover:bg-[#b5952f] hover:text-[#0B1B3D] border-none shadow-xl">
...
for file_name in html_files:
    ...
    content = content.replace("sora", "font-heading")
    
    # 2. Colors replacement (locking to explicit hex codes)
    content = content.replace("bg-gray-900", "bg-[#0B1B3D]")
    content = content.replace("bg-brand-soft", "bg-[#F8FAFC]")
    content = content.replace("bg-gold", "bg-[#D4AF37]")
    content = content.replace("text-gold", "text-[#D4AF37]")
    content = content.replace("border-gold", "border-[#D4AF37]")
    content = content.replace("shadow-gold", "shadow-[#D4AF37]/50")
    
    # Optional context specific
    content = content.replace("bg-subtle-gradient", "bg-white")
    content = content.replace("text-gold-dark", "text-[#b5952f]")
    
    # 3. Footer Replacement
    content = re.sub(r'<!-- FOOTER -->.*?</footer>', footer_new, content, flags=re.DOTALL)
    content = re.sub(r'<!-- SECTION 9 — FOOTER -->.*?</footer>', footer_new, content, flags=re.DOTALL)

    # 4. index.html specific replacements
    if file_name == 'index.html':
        # Hero block
        content = re.sub(r'<!-- SECTION 1 — HERO SECTION -->.*?</section>', hero_new, content, flags=re.DOTALL, count=1)
        # Add Compliance section
        content = content.replace('<!-- SECTION 5 — HOW IT WORKS -->', compliance_new)

    # 5. pricing.html specific replacements
    if file_name == 'pricing.html':
        content = content.replace('<!-- FOOTER -->', enterprise_pricing + '\n\n    ' + '<!-- FOOTER -->')

    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)

print("All HTML files updated successfully with Elite Level brand lock.")
